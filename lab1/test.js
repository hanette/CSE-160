...
function main() {
  ...
  var n = initVertexBuffers_circle(gl,x,y, radius);
  if (n < 0) {
...
function initVertexBuffers_circle(gl, x, y, radius) {
    ...
    for(var circle = 0; circle <= (2*Math.PI); circle += theta){
        vertices[count] = (1/radius)*(x+Math.cos(n*theta));
        count++;
        vertices[count] = (1/radius)*(y+Math.sin(n*theta));
        count++;
        n++;
    }
    n--;
